...
import { 
  ...
  PieChart as PieIcon, LineChart as LineIcon, ChevronRight, Globe, Layers, Boxes
} from 'lucide-react';
import {
...
/**
 * MANDAL GROUP ERP - Premium Enterprise Edition
 * High-Fidelity Manpower & Inventory Management System
 * Features: Persistent LocalStorage, Date-wise Data Segregation, Professional Analytics
 */

const App = () => {
  // --- PERSISTENCE INITIALIZATION ---
  const getInitialSections = () => {
    const saved = localStorage.getItem('mandal_erp_sections');
  ...
  const getInitialDayData = () => {
    const saved = localStorage.getItem('mandal_erp_dayToDayData');
    ...
    return {
      [today]: {
        'SAMBALPUR': {
          manpower: [
            { id: 1, site: 'Sambalpur Main', type: 'Civil Work', required: 20, available: 18 },
            { id: 2, site: 'Sambalpur Phase 2', type: 'Electrical', required: 10, available: 7 }
          ],
          stock: [
            { id: 1, category: "Tools & Measuring", qty: 123 },
            { id: 2, category: "Welding & Cutting", qty: 123 },
            { id: 3, category: "Gas Cutting Items", qty: 423 },
            { id: 4, category: "Electrical Items", qty: 456 },
            { id: 5, category: "Safety Items", qty: 534 },
            { id: 6, category: "Other", qty: 453 }
          ],
          used: [
            { id: 1, name: "Silicon Spray Afra", qty: 1 },
            { id: 2, name: "Taflon Tape", qty: 2 },
            { id: 3, name: "Welding Holder", qty: 3 },
            { id: 4, name: "Leather Handgloves", qty: 4 },
            { id: 5, name: "PVC Tape", qty: 3 }
          ],
          meta: { company: 'Mandal Engineering', location: 'Odisha', siteName: 'Sambalpur Site' }
        }
      }
    };
  };
  ...
  const [dayToDayData, setDayToDayData] = useState(getInitialDayData);
  const [newSectionForm, setNewSectionForm] = useState({ name: '', location: '', siteName: '' });

  // --- PERSISTENCE AUTO-SAVE ---
  useEffect(() => {
    localStorage.setItem('mandal_erp_sections', JSON.stringify(sections));
  }, [sections]);

  useEffect(() => {
    localStorage.setItem('mandal_erp_dayToDayData', JSON.stringify(dayToDayData));
  }, [dayToDayData]);
  ...
  const getDayData = (date, tabId) => {
    const day = dayToDayData[date] || {};
    return day[tabId] || { 
      manpower: [], 
      stock: [
        { id: 1, category: "Tools & Measuring", qty: 0 }, 
        { id: 2, category: "Welding & Cutting", qty: 0 }, 
        { id: 3, category: "Gas Cutting Items", qty: 0 }, 
        { id: 4, category: "Electrical Items", qty: 0 }, 
        { id: 5, category: "Safety Items", qty: 0 }, 
        { id: 6, category: "Other", qty: 0 }
      ], 
      used: [],
  ...
  const updateActiveDayData = (newData) => {
    setDayToDayData(prev => ({
      ...prev,
      [selectedDate]: {
        ...(prev[selectedDate] || {}),
        [activeTab]: newData
  ...
  const currentData = getDayData(selectedDate, activeTab);
  const currentSite = sections.find(s => s.id === activeTab);

  // Totals for Site Page
  const totals = useMemo(() => ({
    required: currentData.manpower.reduce((a, b) => a + (b.required || 0), 0),
    available: currentData.manpower.reduce((a, b) => a + (b.available || 0), 0),
    stock: currentData.stock.reduce((a, b) => a + (b.qty || 0), 0),
    used: currentData.used.reduce((a, b) => a + (b.qty || 0), 0)
  }), [currentData]);
  ...
  // Global Statistics for Summary & Overview
  const summaryStats = useMemo(() => {
    ...
    const sitesData = siteSections.map(site => {
      ...
      const req = data.manpower.reduce((a, b) => a + (b.required || 0), 0);
      const avail = data.manpower.reduce((a, b) => a + (b.available || 0), 0);
      const short = avail - req;
      const att = req > 0 ? (avail / req) * 100 : 0;
      let status = avail >= req ? 'Good' : Math.abs(short) <= 5 ? 'Medium' : 'Critical';
      return { 
        id: site.id, 
        company: data.meta?.company || site.name, 
        siteName: data.meta?.siteName || site.location, 
        location: data.meta?.location || site.location, 
        required: req, 
        available: avail, 
        shortfall: short, 
    ...
    const aggregatedTotals = sitesData.reduce((acc, curr) => ({ 
      required: acc.required + curr.required, 
      available: acc.available + curr.available, 
      shortfall: acc.shortfall + curr.shortfall 
    }), { required: 0, available: 0, shortfall: 0 });
    
    const overallAtt = aggregatedTotals.required > 0 ? (aggregatedTotals.available / aggregatedTotals.required) * 100 : 0;
    const overallStatus = aggregatedTotals.available >= aggregatedTotals.required ? 'Good' : Math.abs(aggregatedTotals.shortfall) <= 5 ? 'Medium' : 'Critical';
    
    return { sites: sitesData, totals: aggregatedTotals, overallAtt: overallAtt.toFixed(1), overallStatus, count: sitesData.length };
  }, [sections, dayToDayData, selectedDate]);
  ...
  const sambalpurMaterial = useMemo(() => {
    ...
    return { 
      stock: data.stock, 
      used: data.used, 
      totalStock: data.stock.reduce((a, b) => a + (b.qty || 0), 0), 
      totalUsed: data.used.reduce((a, b) => a + (b.qty || 0), 0) 
    };
  }, [dayToDayData, selectedDate]);

  // --- ACTIONS ---
  const updateManpower = (id, field, val) => {
    const updated = currentData.manpower.map(r => r.id === id ? { ...r, [field]: (field === 'required' || field === 'available') ? (parseInt(val) || 0) : val } : r);
    updateActiveDayData({ ...currentData, manpower: updated });
  };
  ...
  const updateStock = (id, field, val) => {
    const updated = currentData.stock.map(r => r.id === id ? { ...r, [field]: field === 'qty' ? parseInt(val) || 0 : val } : r);
    updateActiveDayData({ ...currentData, stock: updated });
  };
  ...
  const updateUsed = (id, field, val) => {
    const updated = currentData.used.map(r => r.id === id ? { ...r, [field]: field === 'qty' ? parseInt(val) || 0 : val } : r);
    updateActiveDayData({ ...currentData, used: updated });
  };
  ...
  const updateMeta = (field, val) => updateActiveDayData({ ...currentData, meta: { ...currentData.meta, [field]: val } });
  
  const addSection = () => {
    const id = newSectionForm.name.toUpperCase().replace(/\s+/g, '_');
    setSections([...sections, { id, type: 'site', ...newSectionForm }]);
    setIsAddSectionModalOpen(false);
    setActiveTab(id);
  };
  const deleteSection = (id) => {
    if (window.confirm("Delete this site entirely?")) {
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f9] p-6 lg:p-10 font-['Plus_Jakarta_Sans'] text-slate-700">
        ...
        <div className="relative z-10 mt-8 md:mt-0 flex flex-col items-end gap-4">
          <div className="bg-white/10 backdrop-blur-xl p-5 rounded-3xl border border-white/10 flex flex-col items-end shadow-xl min-w-75">
            ...
            <div className="relative w-full group">
               ...
               <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="w-full bg-white text-[#4e342e] font-black text-sm pl-12 pr-4 py-3 rounded-xl border-2 border-[#4e342e]/10 outline-none cursor-pointer hover:border-[#4e342e]/40 transition-all appearance-none shadow-inner" />
            </div>
          </div>
        </div>
      </header>
      ...
      {activeTab === 'SUMMARY' ? (
        <div className="space-y-10 animate-in fade-in duration-500">
           ...
           <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
              {[
                { l: "Total Sites", v: summaryStats.count, i: Globe, c: COLORS.brown },
                { l: "Total Required", v: summaryStats.totals.required, i: Target, c: COLORS.req },
                { l: "Total Available", v: summaryStats.totals.available, i: UserCheck, c: COLORS.avail },
                { l: "Overall Shortfall", v: summaryStats.totals.shortfall, i: UserX, c: COLORS.short },
                { l: "Overall Attendance", v: `${summaryStats.overallAtt}%`, i: TrendingUp, c: COLORS.att }
              ].map((k, i) => (
                <div key={i} className="bg-white p-6 rounded-[30px] border border-slate-200 shadow-sm flex flex-col items-center group transition-all hover:shadow-xl hover:-translate-y-1">
                   <div className="p-3 rounded-xl mb-4 bg-slate-50 text-slate-500 group-hover:bg-[#4e342e] group-hover:text-white transition-colors"><k.i size={20}/></div>
                   <h2 className="text-3xl font-black mb-1" style={{ color: (k.l.includes('Shortfall') && k.v < 0) ? COLORS.short : k.c }}>{k.v}</h2>
           ...
           <div className="bg-white rounded-[40px] shadow-sm border border-slate-200 overflow-hidden">
              <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                 <h4 className="text-sm font-black text-[#4e342e] uppercase tracking-[0.2em] flex items-center gap-3"><Gauge size={22} /> Executive Performance Summary</h4>
              </div>
              <div className="overflow-x-auto">
                 <table className="w-full text-left">
                    <thead><tr className="bg-slate-50 border-b border-slate-200">{['Sr.No', 'Company Name', 'Site Name', 'Total Required', 'Total Available', 'Shortfall', 'Attendance %', 'Status'].map(h => (<th key={h} className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">{h}</th>))}</tr></thead>
                    <tbody className="divide-y divide-slate-100">
                       {summaryStats.sites.map((site, i) => (<tr key={site.id} className="hover:bg-slate-50/50 transition-all cursor-pointer" onClick={() => setActiveTab(site.id)}><td className="px-8 py-6 text-center font-black text-slate-300">{i + 1}</td><td className="px-8 py-6 text-center font-bold text-[#4e342e]">{site.company}</td><td className="px-8 py-6 text-center font-bold text-slate-600">{site.siteName}</td><td className="px-8 py-6 text-center font-black">{site.required}</td><td className="px-8 py-6 text-center font-black">{site.available}</td><td className={`px-8 py-6 text-center font-black ${site.shortfall < 0 ? 'text-rose-500' : 'text-emerald-500'}`}>{site.shortfall}</td><td className="px-8 py-6 text-center font-black text-slate-600">{site.attendance}%</td><td className="px-8 py-6 text-center"><span className="px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest text-white shadow-lg" style={{ backgroundColor: site.status === 'Good' ? COLORS.avail : site.status === 'Medium' ? COLORS.req : COLORS.short }}>{site.status}</span></td></tr>))}
                    </tbody>
                    <tfoot className="sticky bottom-0 bg-[#4e342e] text-white border-t-4 border-white/20 shadow-[0_-10px_20px_rgba(0,0,0,0.1)]">
                       <tr className="font-black">
                          <td colSpan="3" className="px-8 py-6 text-center uppercase tracking-[0.3em] text-[11px] border-r border-white/10">Organization Totals</td>
                          <td className="px-8 py-6 text-center text-xl border-r border-white/10">{summaryStats.totals.required}</td>
                          <td className="px-8 py-6 text-center text-xl border-r border-white/10">{summaryStats.totals.available}</td>
                          <td className={`px-8 py-6 text-center text-xl border-r border-white/10 ${summaryStats.totals.shortfall < 0 ? 'text-rose-400' : 'text-emerald-400'}`}>{summaryStats.totals.shortfall}</td>
                          <td className="px-8 py-6 text-center text-xl border-r border-white/10">{summaryStats.overallAtt}%</td>
                          <td className="px-8 py-6 text-center"><span className="px-6 py-2 rounded-xl text-[10px] uppercase tracking-widest bg-white/20 border border-white/30">{summaryStats.overallStatus}</span></td>
                       </tr>
                    </tfoot>
                 </table>
              </div>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
              <div className="bg-white p-10 rounded-[50px] border border-slate-200 shadow-sm h-110 group hover:shadow-2xl transition-all">
                 <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-10 flex items-center gap-2"><BarChart3 size={16} /> Site Manpower Benchmarking</h4>
                 <ResponsiveContainer width="100%" height="80%">
                    <BarChart data={summaryStats.sites}>
                       <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                       <XAxis dataKey="id" tick={{fontSize: 10, fontWeight: 800}} />
                       <YAxis hide />
                       <Tooltip contentStyle={{borderRadius: '20px', border: 'none', boxShadow: '0 20px 40px rgba(0,0,0,0.1)'}} />
                       <Legend iconType="circle" wrapperStyle={{paddingTop: '20px', fontSize: '10px', fontWeight: 'bold'}} />
                       <Bar name="Required" dataKey="required" fill={COLORS.req} radius={[10, 10, 0, 0]} />
                       <Bar name="Available" dataKey="available" fill={COLORS.avail} radius={[10, 10, 0, 0]} />
                    </BarChart>
                 </ResponsiveContainer>
              </div>
              <div className="bg-white p-10 rounded-[50px] border border-slate-200 shadow-sm h-110 group hover:shadow-2xl transition-all">
                 <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-10 flex items-center gap-2"><PieIcon size={16} /> Organization Fulfilment</h4>
                 <ResponsiveContainer width="100%" height="80%">
                    <PieChart>
                       <Pie data={[{name: 'Available', value: summaryStats.totals.available}, {name: 'Shortfall', value: Math.abs(summaryStats.totals.shortfall)}]} innerRadius={70} outerRadius={100} paddingAngle={8} dataKey="value"><Cell fill={COLORS.avail} /><Cell fill={COLORS.short} /></Pie>
                       <Tooltip /><Legend verticalAlign="bottom" align="center" wrapperStyle={{fontSize: '10px', fontWeight: 'bold'}} />
                    </PieChart>
                 </ResponsiveContainer>
              </div>
              <div className="bg-white p-10 rounded-[50px] border border-slate-200 shadow-sm h-110 group hover:shadow-2xl transition-all">
                 <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-10 flex items-center gap-2"><LineIcon size={16} /> Attendance Velocity</h4>
                 <ResponsiveContainer width="100%" height="80%">
                    <AreaChart data={summaryStats.sites.map(s => ({ n: s.id, v: parseFloat(s.attendance) }))}>
                       <defs><linearGradient id="summAttGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={COLORS.att} stopOpacity={0.3}/><stop offset="95%" stopColor={COLORS.att} stopOpacity={0}/></linearGradient></defs>
                       <XAxis dataKey="n" tick={{fontSize: 10, fontWeight: 800}} /><YAxis domain={[0, 100]} hide /><Tooltip />
                       <Area type="monotone" name="Attendance %" dataKey="v" stroke={COLORS.att} strokeWidth={4} fillOpacity={1} fill="url(#summAttGrad)" />
                    </AreaChart>
                 </ResponsiveContainer>
              </div>
           </div>
        ...
        <div className="space-y-12 animate-in fade-in duration-700">
           <div className="grid grid-cols-1 md:grid-cols-5 gap-6">{[{ l: "Active Entities", v: summaryStats.count, i: Building2, c: COLORS.brown }, { l: "Projected Manpower", v: summaryStats.totals.required, i: Target, c: COLORS.req }, { l: "Deployed Force", v: summaryStats.totals.available, i: UserCheck, c: COLORS.avail }, { l: "Organization Gap", v: summaryStats.totals.shortfall, i: UserX, c: COLORS.short }, { l: "Average Attendance", v: `${summaryStats.overallAtt}%`, i: TrendingUp, c: COLORS.att }].map((k, i) => (<div key={i} className="bg-white p-7 rounded-[35px] border border-white shadow-sm flex flex-col items-center group hover:shadow-xl transition-all"><div className="p-3.5 rounded-2xl mb-4 bg-slate-50 text-slate-400 group-hover:bg-[#4e342e] group-hover:text-white transition-all"><k.i size={20}/></div><h2 className="text-3xl font-black mb-1" style={{ color: (k.l.includes('Gap') && k.v < 0) ? COLORS.short : k.c }}>{k.v}</h2><span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-2">{k.l}</span></div>))}</div>
      ...
      <footer className="mt-10 border-t border-slate-200 pt-10 flex flex-col md:flex-row justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">
        <div>© 2026 Mandal Group • ERP Infrastructure v15.2.0</div>
