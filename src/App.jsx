...
const App = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(configRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setDoc(configRef, { sections: defaults });
      }
      setIsLoading(false);
    });
    return () => unsub();
  ...
  const getDayData = useCallback((date, tabId) => {
    ...
    // Default Tables for SAMBALPUR Material Status
  ...
  const summaryStats = useMemo(() => {
    ...
    const ovAtt = agg.required > 0 ? (agg.available / agg.required * 100).toFixed(1) : 0;
    const ovStat = agg.available >= agg.required ? 'Good' : 'Critical';
    return { sites: sitesData, totals: agg, overallAtt: ovAtt, overallStatus: ovStat, count: sitesData.length };
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f9] p-6 lg:p-10 font-['Plus_Jakarta_Sans'] text-slate-700">
      ...
      {editingSection && (
        <div className="fixed inset-0 z-99999 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[40px] p-10 w-full max-w-lg shadow-2xl scale-in-center border border-white/20">
             <div className="flex justify-between items-center mb-6"><h3 className="text-2xl font-black text-[#4e342e]">Edit Section Details</h3><button onClick={() => setEditingSection(null)} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X size={24} /></button></div>
             <div className="space-y-4">
                <div><label className="text-[10px] font-black uppercase text-slate-400 ml-1">Company Name</label><input value={editingSection.name} onChange={e => setEditingSection({...editingSection, name: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none focus:border-[#4e342e] font-bold" /></div>
                <div><label className="text-[10px] font-black uppercase text-slate-400 ml-1">Location</label><input value={editingSection.location} onChange={e => setEditingSection({...editingSection, location: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none focus:border-[#4e342e] font-bold" /></div>
                <div><label className="text-[10px] font-black uppercase text-slate-400 ml-1">Site Name</label><input value={editingSection.siteName} onChange={e => setEditingSection({...editingSection, siteName: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none focus:border-[#4e342e] font-bold" /></div>
                <div><label className="text-[10px] font-black uppercase text-slate-400 ml-1">Tab ID</label><input value={editingSection.id} onChange={e => setEditingSection({...editingSection, id: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none focus:border-[#4e342e] font-bold" /></div>
             </div>
             <div className="flex gap-4 mt-8"><button onClick={() => setEditingSection(null)} className="flex-1 py-4 font-bold text-slate-400">Cancel</button><button onClick={saveEditedSection} className="flex-1 py-4 bg-[#4e342e] text-white rounded-2xl font-black uppercase tracking-widest shadow-xl">Save Changes</button></div>
          </div>
        </div>
      )}
      ...
      {isAddSectionModalOpen && (
        <div className="fixed inset-0 z-99999 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[40px] p-10 w-full max-w-lg shadow-2xl border border-white/20 scale-in-center">
             ...
             <div className="space-y-4">
                <div><label className="text-[10px] font-black uppercase text-slate-400 ml-1">Company</label><input value={newSectionForm.name} onChange={e => setNewSectionForm({...newSectionForm, name: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none focus:border-[#4e342e] font-bold" /></div>
      ...
      <header className="relative bg-linear-to-br from-[#8d6e63] via-[#4e342e] to-[#3e2723] rounded-[40px] p-8 lg:p-12 mb-10 shadow-2xl flex flex-col md:flex-row justify-between items-center text-white border border-white/10 overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-end gap-4">
          <div className="bg-white/10 backdrop-blur-xl p-5 rounded-3xl border border-white/10 flex flex-col items-end shadow-xl min-w-75">
            ...
            <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="w-full bg-white text-[#4e342e] font-black text-sm px-4 py-3 rounded-xl border-2 border-[#4e342e]/10 outline-none" />
          </div>
        </div>
      </header>
      ...
      {activeTab === 'SUMMARY' ? (
        ...
        <div className="space-y-10 animate-in fade-in duration-500">
           ...
           <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              {activeTab === 'SAMBALPUR' && <button onClick={() => setSambalpurView(sambalpurView === 'material' ? 'deployment' : 'material')} className={`px-8 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all shadow-xl ${sambalpurView === 'material' ? 'bg-[#4e342e] text-white' : 'bg-white text-[#4e342e] border border-slate-200 hover:bg-slate-50'}`}>{sambalpurView === 'material' ? 'Deployment View' : 'Material Hub'}</button>}
           </div>

           {sambalpurView === 'material' && activeTab === 'SAMBALPUR' ? (
              <div className="space-y-12">
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="bg-white rounded-[40px] shadow-sm border border-slate-200 overflow-hidden"><div className="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center"><h4 className="text-sm font-black text-[#4e342e] uppercase tracking-[0.2em] flex items-center gap-3"><Package size={22} /> Closing Stock</h4>{isAdminMode && <button onClick={() => updateActiveDayData({...currentData, stock: [...currentData.stock, {id: Date.now(), category: 'New', qty: 0}]})} className="p-2 bg-[#4e342e] text-white rounded-lg"><Plus size={14}/></button>}</div><div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="bg-slate-50 text-[10px] font-black text-slate-400"><th className="px-8 py-4">Sr No</th><th className="px-8 py-4">Category</th><th className="px-8 py-4 text-center">Quantity</th>{isAdminMode && <th className="px-8 py-4 text-center">Actions</th>}</tr></thead><tbody className="divide-y divide-slate-50">{currentData.stock.map((m, i) => (<tr key={m.id} className="hover:bg-slate-50/50"><td className="px-8 py-4 font-black text-slate-300">{i+1}</td><td className="px-8 py-4 font-bold text-[#4e342e]">{isAdminMode ? <input value={m.category} onChange={e => updateStock(m.id, 'category', e.target.value)} className="bg-slate-100 p-1 rounded w-full outline-none" /> : m.category}</td><td className="px-8 py-4 text-center font-black text-[#4e342e]">{isAdminMode ? <input type="number" value={m.qty} onChange={e => updateStock(m.id, 'qty', e.target.value)} className="bg-slate-100 p-1 rounded w-20 text-center outline-none" /> : m.qty}</td>{isAdminMode && <td className="px-8 py-4 text-center"><button onClick={() => updateActiveDayData({...currentData, stock: currentData.stock.filter(x => x.id !== m.id)})} className="text-rose-500"><Trash2 size={16}/></button></td>}</tr>))}</tbody></table></div></div>
                    <div className="bg-white rounded-[40px] shadow-sm border border-slate-200 overflow-hidden"><div className="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center"><h4 className="text-sm font-black text-[#4e342e] uppercase tracking-[0.2em] flex items-center gap-3"><ClipboardList size={22} /> Material Used</h4>{isAdminMode && <button onClick={() => updateActiveDayData({...currentData, used: [...currentData.used, {id: Date.now(), name: 'New', qty: 0}]})} className="p-2 bg-[#4e342e] text-white rounded-lg"><Plus size={14}/></button>}</div><div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="bg-slate-50 text-[10px] font-black text-slate-400"><th className="px-8 py-4">Sr No</th><th className="px-8 py-4">Material Name</th><th className="px-8 py-4 text-center">Quantity</th>{isAdminMode && <th className="px-8 py-4 text-center">Actions</th>}</tr></thead><tbody className="divide-y divide-slate-50">{currentData.used.map((m, i) => (<tr key={m.id} className="hover:bg-slate-50/50"><td className="px-8 py-4 font-black text-slate-300">{i+1}</td><td className="px-8 py-4 font-bold text-[#4e342e]">{isAdminMode ? <input value={m.name} onChange={e => updateUsed(m.id, 'name', e.target.value)} className="bg-slate-100 p-1 rounded w-full outline-none" /> : m.name}</td><td className="px-8 py-4 text-center font-black text-[#4e342e]">{isAdminMode ? <input type="number" value={m.qty} onChange={e => updateUsed(m.id, 'qty', e.target.value)} className="bg-slate-100 p-1 rounded w-20 text-center outline-none" /> : m.qty}</td>{isAdminMode && <td className="px-8 py-4 text-center"><button onClick={() => updateActiveDayData({...currentData, used: currentData.used.filter(x => x.id !== m.id)})} className="text-rose-500"><Trash2 size={16}/></button></td>}</tr>))}</tbody></table></div></div>
                 </div>
                 
                 <div className="bg-white rounded-[40px] shadow-sm border border-slate-200 overflow-hidden"><div className="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center"><h4 className="text-sm font-black text-[#4e342e] uppercase tracking-[0.2em] flex items-center gap-3"><Wrench size={22} /> Equipment Status</h4>{isAdminMode && <button onClick={() => updateActiveDayData({...currentData, equipmentStatus: [...currentData.equipmentStatus, {id: Date.now(), name: 'New', qty: 0}]})} className="p-2 bg-[#4e342e] text-white rounded-lg"><Plus size={14}/></button>}</div><div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="bg-slate-50 text-[10px] font-black text-slate-400"><th className="px-8 py-4">Sr No</th><th className="px-8 py-4">Equipment Name</th><th className="px-8 py-4 text-center">Quantity</th>{isAdminMode && <th className="px-8 py-4 text-center">Actions</th>}</tr></thead><tbody className="divide-y divide-slate-50">{currentData.equipmentStatus.map((m, i) => (<tr key={m.id} className="hover:bg-slate-50/50 transition-all"><td className="px-8 py-4 font-black text-slate-300">{i+1}</td><td className="px-8 py-4 font-bold text-[#4e342e]">{isAdminMode ? <input value={m.name} onChange={e => updateEquipment(m.id, 'name', e.target.value)} className="bg-slate-100 p-1 rounded w-full outline-none" /> : m.name}</td><td className="px-8 py-4 text-center font-black text-[#4e342e]">{isAdminMode ? <input type="number" value={m.qty} onChange={e => updateEquipment(m.id, 'qty', e.target.value)} className="bg-slate-100 p-1 rounded w-20 text-center outline-none" /> : m.qty}</td>{isAdminMode && <td className="px-8 py-4 text-center"><button onClick={() => updateActiveDayData({...currentData, equipmentStatus: currentData.equipmentStatus.filter(x => x.id !== m.id)})} className="text-rose-500"><Trash2 size={16}/></button></td>}</tr>))}</tbody></table></div></div>
                 
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-8"><div className="bg-white rounded-[40px] shadow-sm border border-slate-200 overflow-hidden"><div className="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center"><h4 className="text-sm font-black text-[#4e342e] uppercase tracking-[0.2em] flex items-center gap-3"><ActivityIcon size={22} /> Work Progress</h4>{isAdminMode && <button onClick={() => updateActiveDayData({...currentData, workProgress: [...currentData.workProgress, {id: Date.now(), name: 'New', value: '-'}]})} className="p-2 bg-[#4e342e] text-white rounded-lg"><Plus size={14}/></button>}</div><div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="bg-slate-50 text-[10px] font-black text-slate-400"><th className="px-8 py-4">Sr No</th><th className="px-8 py-4">Work Type</th><th className="px-8 py-4 text-center">Status</th>{isAdminMode && <th className="px-8 py-4 text-center">Actions</th>}</tr></thead><tbody className="divide-y divide-slate-50">{currentData.workProgress.map((m, i) => (<tr key={m.id} className="hover:bg-slate-50/50 transition-all"><td className="px-8 py-4 font-black text-slate-300">{i+1}</td><td className="px-8 py-4 font-bold text-[#4e342e]">{isAdminMode ? <input value={m.name} onChange={e => updateWork(m.id, 'name', e.target.value)} className="bg-slate-100 p-1 rounded w-full outline-none" /> : m.name}</td><td className="px-8 py-4 text-center font-bold text-slate-600">{isAdminMode ? <input value={m.value} onChange={e => updateWork(m.id, 'value', e.target.value)} className="bg-slate-100 p-1 rounded w-full outline-none text-center" /> : m.value}</td>{isAdminMode && <td className="px-8 py-4 text-center"><button onClick={() => updateActiveDayData({...currentData, workProgress: currentData.workProgress.filter(x => x.id !== m.id)})} className="text-rose-500"><Trash2 size={16}/></button></td>}</tr>))}</tbody></table></div></div><div className="bg-white rounded-[40px] shadow-sm border border-slate-200 overflow-hidden"><div className="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center"><h4 className="text-sm font-black text-[#4e342e] uppercase tracking-[0.2em] flex items-center gap-3"><UserPlus size={22} /> Manpower Detail</h4>{isAdminMode && <button onClick={() => updateActiveDayData({...currentData, manpowerDetail: [...currentData.manpowerDetail, {id: Date.now(), name: 'New', qty: 0}]})} className="p-2 bg-[#4e342e] text-white rounded-lg"><Plus size={14}/></button>}</div><div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="bg-slate-50 text-[10px] font-black text-slate-400"><th className="px-8 py-4">Sr No</th><th className="px-8 py-4">Detail</th><th className="px-8 py-4 text-center">Qty</th>{isAdminMode && <th className="px-8 py-4 text-center">Actions</th>}</tr></thead><tbody className="divide-y divide-slate-50">{currentData.manpowerDetail.map((m, i) => (<tr key={m.id} className="hover:bg-slate-50/50 transition-all"><td className="px-8 py-4 font-black text-slate-300">{i+1}</td><td className="px-8 py-4 font-bold text-[#4e342e]">{isAdminMode ? <input value={m.name} onChange={e => updateManpowerDetail(m.id, 'name', e.target.value)} className="bg-slate-100 p-1 rounded w-full outline-none" /> : m.name}</td><td className="px-8 py-4 text-center font-black text-[#4e342e]">{isAdminMode ? <input type="number" value={m.qty} onChange={e => updateManpowerDetail(m.id, 'qty', e.target.value)} className="bg-slate-100 p-1 rounded w-20 text-center outline-none" /> : m.qty}</td>{isAdminMode && <td className="px-8 py-4 text-center"><button onClick={() => updateActiveDayData({...currentData, manpowerDetail: currentData.manpowerDetail.filter(x => x.id !== m.id)})} className="text-rose-500"><Trash2 size={16}/></button></td>}</tr>))}</tbody></table></div></div></div>
              </div>
           ) : (
              <>
      ...
      <footer className="mt-10 border-t border-slate-200 pt-10 flex flex-col md:flex-row justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">
        <div>© 2026 Mandal Group • Cloud ERP v18.2.0</div>
