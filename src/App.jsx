...
import { 
  ...
  Wrench, Activity as ActivityIcon, UserPlus
} from 'lucide-react';
import {
...
const App = () => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "mandal_erp", "config"), (docSnap) => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "daily_reports", selectedDate), (docSnap) => {
      if (docSnap.exists()) setDayToDayData(prev => ({ ...prev, [selectedDate]: docSnap.data().sites || {} }));
      else setDayToDayData(prev => ({ ...prev, [selectedDate]: {} }));
    });
    return () => unsub();
  }, [selectedDate]);
  ...
  const handleAuth = () => {
    if (password === "D@1010") {
      setIsAdminMode(true);
      setIsAuthModalOpen(false); // CRITICAL: Close modal
      setPassword("");           // CRITICAL: Reset input
    } else {
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-[#4e342e]" size={40}/></div>;
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f9] p-6 lg:p-10 font-['Plus_Jakarta_Sans'] text-slate-700">
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl p-10 w-full max-w-sm shadow-2xl scale-in-center">
            ...
            <input 
              ...
              className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl mb-6 text-center outline-none focus:border-[#8d6e63]" 
              placeholder="••••" 
            />
            <div className="flex gap-4">
              <button onClick={() => { setIsAuthModalOpen(false); setPassword(""); }} className="flex-1 py-3 font-bold text-slate-400">Cancel</button>
              <button onClick={handleAuth} className="flex-1 py-3 bg-[#4e342e] text-white rounded-xl font-bold hover:bg-[#3e2723] transition-all">Verify</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3 mb-10 bg-white/60 backdrop-blur-md p-3 rounded-4xl border border-white shadow-sm overflow-hidden">
        {sections.map((sec) => (
          <div key={sec.id} className="relative group">
            ...
            {isAdminMode && sec.type === 'site' && <button onClick={() => deleteSection(sec.id)} className="absolute -top-2 -right-2 p-1.5 bg-rose-500 text-white rounded-full opacity-0 group-hover:opacity-100 shadow-lg"><Trash2 size={12}/></button>}
          </div>
        ))}
           ...
           {sambalpurView === 'material' && activeTab === 'SAMBALPUR' ? (
              <div className="space-y-12">
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="bg-white rounded-[40px] shadow-sm border border-slate-200 overflow-hidden"><div className="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center"><h4 className="text-sm font-black text-[#4e342e] uppercase tracking-[0.2em] flex items-center gap-3"><Package size={22} /> Closing Stock</h4>{isAdminMode && <button onClick={() => updateActiveDayData({...currentData, stock: [...currentData.stock, {id: Date.now(), category: 'New Category', qty: 0}]})} className="p-2 bg-[#4e342e] text-white rounded-lg"><Plus size={14}/></button>}</div><div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="bg-slate-50 text-[10px] font-black text-slate-400"><th className="px-8 py-4">Sr No</th><th className="px-8 py-4">Category</th><th className="px-8 py-4 text-center">Quantity</th>{isAdminMode && <th className="px-8 py-4 text-center">Actions</th>}</tr></thead><tbody className="divide-y divide-slate-50">{currentData.stock.map((m, i) => (<tr key={m.id} className="hover:bg-slate-50/50 transition-all"><td className="px-8 py-4 font-black text-slate-300">{i+1}</td><td className="px-8 py-4 font-bold text-[#4e342e]">{isAdminMode ? <input value={m.category} onChange={e => updateStock(m.id, 'category', e.target.value)} className="bg-slate-100 p-1 rounded w-full outline-none" /> : m.category}</td><td className="px-8 py-4 text-center font-black text-[#4e342e]">{isAdminMode ? <input type="number" value={m.qty} onChange={e => updateStock(m.id, 'qty', e.target.value)} className="bg-slate-100 p-1 rounded w-20 text-center outline-none" /> : m.qty}</td>{isAdminMode && <td className="px-8 py-4 text-center"><button onClick={() => updateActiveDayData({...currentData, stock: currentData.stock.filter(x => x.id !== m.id)})} className="text-rose-500"><Trash2 size={16}/></button></td>}</tr>))}</tbody></table></div></div>
                    <div className="bg-white rounded-[40px] shadow-sm border border-slate-200 overflow-hidden"><div className="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center"><h4 className="text-sm font-black text-[#4e342e] uppercase tracking-[0.2em] flex items-center gap-3"><ClipboardList size={22} /> Material Used</h4>{isAdminMode && <button onClick={() => updateActiveDayData({...currentData, used: [...currentData.used, {id: Date.now(), name: 'New Material', qty: 0}]})} className="p-2 bg-[#4e342e] text-white rounded-lg"><Plus size={14}/></button>}</div><div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="bg-slate-50 text-[10px] font-black text-slate-400"><th className="px-8 py-4">Sr No</th><th className="px-8 py-4">Material Name</th><th className="px-8 py-4 text-center">Quantity</th>{isAdminMode && <th className="px-8 py-4 text-center">Actions</th>}</tr></thead><tbody className="divide-y divide-slate-50">{currentData.used.map((m, i) => (<tr key={m.id} className="hover:bg-slate-50/50 transition-all"><td className="px-8 py-4 font-black text-slate-300">{i+1}</td><td className="px-8 py-4 font-bold text-[#4e342e]">{isAdminMode ? <input value={m.name} onChange={e => updateUsed(m.id, 'name', e.target.value)} className="bg-slate-100 p-1 rounded w-full outline-none" /> : m.name}</td><td className="px-8 py-4 text-center font-black text-[#4e342e]">{isAdminMode ? <input type="number" value={m.qty} onChange={e => updateUsed(m.id, 'qty', e.target.value)} className="bg-slate-100 p-1 rounded w-20 text-center outline-none" /> : m.qty}</td>{isAdminMode && <td className="px-8 py-4 text-center"><button onClick={() => updateActiveDayData({...currentData, used: currentData.used.filter(x => x.id !== m.id)})} className="text-rose-500"><Trash2 size={16}/></button></td>}</tr>))}</tbody></table></div></div>
                 </div>

                 {/* NEW TABLES FOR SAMBALPUR MATERIAL VIEW */}
                 <div className="bg-white rounded-[40px] shadow-sm border border-slate-200 overflow-hidden">
                    <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center"><h4 className="text-sm font-black text-[#4e342e] uppercase tracking-[0.2em] flex items-center gap-3"><Wrench size={22} /> Equipment Status</h4>{isAdminMode && <button onClick={() => updateActiveDayData({...currentData, equipmentStatus: [...currentData.equipmentStatus, {id: Date.now(), name: 'New Equipment', qty: 0}]})} className="p-2 bg-[#4e342e] text-white rounded-lg"><Plus size={14}/></button>}</div>
                    <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="bg-slate-50 text-[10px] font-black text-slate-400"><th className="px-8 py-4">Sr No</th><th className="px-8 py-4">Equipment Name</th><th className="px-8 py-4 text-center">Quantity</th>{isAdminMode && <th className="px-8 py-4 text-center">Actions</th>}</tr></thead>
                    <tbody className="divide-y divide-slate-50">{currentData.equipmentStatus.map((m, i) => (<tr key={m.id} className="hover:bg-slate-50/50 transition-all"><td className="px-8 py-4 font-black text-slate-300">{i+1}</td><td className="px-8 py-4 font-bold text-[#4e342e]">{isAdminMode ? <input value={m.name} onChange={e => updateEquipment(m.id, 'name', e.target.value)} className="bg-slate-100 p-1 rounded w-full outline-none" /> : m.name}</td><td className="px-8 py-4 text-center font-black text-[#4e342e]">{isAdminMode ? <input type="number" value={m.qty} onChange={e => updateEquipment(m.id, 'qty', e.target.value)} className="bg-slate-100 p-1 rounded w-20 text-center outline-none" /> : m.qty}</td>{isAdminMode && <td className="px-8 py-4 text-center"><button onClick={() => updateActiveDayData({...currentData, equipmentStatus: currentData.equipmentStatus.filter(x => x.id !== m.id)})} className="text-rose-500"><Trash2 size={16}/></button></td>}</tr>))}</tbody></table></div>
                 </div>

                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="bg-white rounded-[40px] shadow-sm border border-slate-200 overflow-hidden"><div className="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center"><h4 className="text-sm font-black text-[#4e342e] uppercase tracking-[0.2em] flex items-center gap-3"><ActivityIcon size={22} /> Work Progress</h4>{isAdminMode && <button onClick={() => updateActiveDayData({...currentData, workProgress: [...currentData.workProgress, {id: Date.now(), name: 'New Work', value: ''}]})} className="p-2 bg-[#4e342e] text-white rounded-lg"><Plus size={14}/></button>}</div><div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="bg-slate-50 text-[10px] font-black text-slate-400"><th className="px-8 py-4">Sr No</th><th className="px-8 py-4">Work Type</th><th className="px-8 py-4 text-center">Progress / Status</th>{isAdminMode && <th className="px-8 py-4 text-center">Actions</th>}</tr></thead>
                    <tbody className="divide-y divide-slate-50">{currentData.workProgress.map((m, i) => (<tr key={m.id} className="hover:bg-slate-50/50 transition-all"><td className="px-8 py-4 font-black text-slate-300">{i+1}</td><td className="px-8 py-4 font-bold text-[#4e342e]">{isAdminMode ? <input value={m.name} onChange={e => updateWork(m.id, 'name', e.target.value)} className="bg-slate-100 p-1 rounded w-full outline-none" /> : m.name}</td><td className="px-8 py-4 text-center font-bold text-slate-600">{isAdminMode ? <input value={m.value} onChange={e => updateWork(m.id, 'value', e.target.value)} className="bg-slate-100 p-1 rounded w-full outline-none text-center" /> : m.value}</td>{isAdminMode && <td className="px-8 py-4 text-center"><button onClick={() => updateActiveDayData({...currentData, workProgress: currentData.workProgress.filter(x => x.id !== m.id)})} className="text-rose-500"><Trash2 size={16}/></button></td>}</tr>))}</tbody></table></div></div>
                    ...
                    <tbody className="divide-y divide-slate-50">{currentData.manpowerDetail.map((m, i) => (<tr key={m.id} className="hover:bg-slate-50/50 transition-all"><td className="px-8 py-4 font-black text-slate-300">{i+1}</td><td className="px-8 py-4 font-bold text-[#4e342e]">{isAdminMode ? <input value={m.name} onChange={e => updateManpowerDetail(m.id, 'name', e.target.value)} className="bg-slate-100 p-1 rounded w-full outline-none" /> : m.name}</td><td className="px-8 py-4 text-center font-black text-[#4e342e]">{isAdminMode ? <input type="number" value={m.qty} onChange={e => updateManpowerDetail(m.id, 'qty', e.target.value)} className="bg-slate-100 p-1 rounded w-20 text-center outline-none" /> : m.qty}</td>{isAdminMode && <td className="px-8 py-4 text-center"><button onClick={() => updateActiveDayData({...currentData, manpowerDetail: currentData.manpowerDetail.filter(x => x.id !== m.id)})} className="text-rose-500"><Trash2 size={16}/></button></td>}</tr>))}</tbody></table></div></div>
                 </div>
              </div>
           ) : (
              <>
      ...
      <footer className="mt-10 border-t border-slate-200 pt-10 flex flex-col md:flex-row justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">
        <div>© 2026 Mandal Group • Real-Time Cloud ERP v17.5.0</div>
